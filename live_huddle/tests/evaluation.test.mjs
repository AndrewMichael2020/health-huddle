...
test("accepts a complete grounded six-agent run", () => {
  ...
  const events = [
    ...Array.from({length:5}, () => ({type:"agent_tool_response",agent_tool_response:{tool_name:"transfer_to_agent"}})),
    ...Array.from({length:5}, () => ({type:"client_tool_call",client_tool_call:{tool_name:"yield_floor"}}))
  ];
...
test("rejects speaking turns that transfer but never yield", () => {
  const contribution = "Four deliveries repeat earlier events. Referral, status, and event time form a candidate key; the PARIS row ID is lineage. The Human source owner must confirm uniqueness and timestamps. I can help cross-check the written evidence. I yield my time.";
  const transcript = ["daniel","priya","marcus","elena","owen"].map((speaker) => ({speaker,kind:"contribution",text:speaker === "daniel" ? `${contribution} The modernization goal is a governed Fabric foundation under human authority.` : contribution}));
  transcript.push({speaker:"maya",kind:"closing",text:"The sample supports a candidate PARIS event grain while row IDs remain lineage. Human owners retain their decisions, and no Human approval was given. Happy Wednesday, everyone."});
  const floorEvents = ["daniel","priya","marcus","elena","owen"].flatMap((agent_id) => ["floor_granted","speaking","yielded"].map((state) => ({agent_id,state})));
  const events = Array.from({length:5}, () => ({type:"agent_tool_response",agent_tool_response:{tool_name:"transfer_to_agent"}}));
  const result = evaluateRun({transcript, investigations:{reports:{elena:{}},marcus_followup:{}}, events, floorEvents, projectAction:{requested:true,result:{changed:true}}, projectRestored:true, auditText:"clean", durationSeconds:300, creditsUsed:1000, creditCeiling:10000});
  assert.equal(result.criteria.find((item) => item.id === "AC-03").passed, false);
});

test("rejects a duplicate outbound transfer for one speaking turn", () => {
  const contribution = "Four deliveries repeat earlier events. Referral, status, and event time form a candidate key; the PARIS row ID is lineage. The Human source owner must confirm uniqueness and timestamps. I can help cross-check the written evidence. I yield my time.";
  const transcript = ["daniel","priya","marcus","elena","owen"].map((speaker) => ({speaker,kind:"contribution",text:speaker === "daniel" ? `${contribution} The modernization goal is a governed Fabric foundation under human authority.` : contribution}));
  transcript.push({speaker:"maya",kind:"closing",text:"The sample supports a candidate PARIS event grain while row IDs remain lineage. Human owners retain their decisions, and no Human approval was given. Happy Wednesday, everyone."});
  const floorEvents = ["daniel","priya","marcus","elena","owen"].flatMap((agent_id) => ["floor_granted","speaking","yielded"].map((state) => ({agent_id,state})));
  const events = [
    ...Array.from({length:6}, () => ({type:"agent_tool_response",agent_tool_response:{tool_name:"transfer_to_agent"}})),
    ...Array.from({length:5}, () => ({type:"client_tool_call",client_tool_call:{tool_name:"yield_floor"}}))
  ];
  const result = evaluateRun({transcript, investigations:{reports:{elena:{}},marcus_followup:{}}, events, floorEvents, projectAction:{requested:true,result:{changed:true}}, projectRestored:true, auditText:"clean", durationSeconds:300, creditsUsed:1000, creditCeiling:10000});
  assert.equal(result.criteria.find((item) => item.id === "AC-03").passed, false);
